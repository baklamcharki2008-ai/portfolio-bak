(() => {
  ...
  function a(t) {
    ...
    if (i) { $('.sun', i).style.display = e ? 'block' : 'none'; $('.moon', i).style.display = e ? 'none' : 'block' }
  ...
  R('.feature-item').forEach(btn => btn.addEventListener('click', () => {
    ...
    setTimeout(() => { o.innerHTML = `<span>${a}</strong><small>${c}</small>`; o.style.opacity = '1'; }, 180);
  ...
  R('.arch-tab').forEach(tab => tab.addEventListener('click', () => {
    ...
    const d = arch[tab.dataset.layer];
    const diag = r('#archDiagram'), desc = r('#archDescription');
    diag.innerHTML = d.nodes.map((n, i) => `<div class="arch-node"><span>${n[0]}</small><small>${n[1]}</small></div>${i < d.nodes.length - 1 ? '<div class="arch-arrow">↓</div>' : ''}`).join('');
    desc.innerHTML = `<h3>${d.title}</h3><p>${d.desc}</p><ul>${d.nodes.map(n => `<li>${n[0]} <span>— ${n[1]}</span></li>`).join('')}</ul>`;
  }));
  R('.arch-tab').forEach(tab => tab.addEventListener('click', () => renderArch(tab.dataset.layer)));
